...
function App() {
  ...
  const processAudio = () => {
    ...
    for (let i = 0; i < dataArray.length; i++) {
        let val = (dataArray[i] - 128) / 128; // Normalize -1 to 1
        sum += val * val;
    }
    let rms = Math.sqrt(sum / dataArray.length);

    // Filter out very low noise, then boost
    const targetVolume = rms > 0.015 ? Math.min(rms * 6, 1.0) : 0;
    
    // Smooth the volume (interpolation) to prevent jitter
    ...
    <div className="app-container">
      ...
      <div className="controls-panel">
        <div className="controls-header">
          <h1>Live VTuber</h1>
          <p>Smart Lip-Sync via Web Audio API</p>
        </div>

        <div className="controls-grid">
          <div className="control-group">
            <label>Avatar Seed (Genetics)</label>
            <input 
              type="text" 
              className="input-field" 
              value={seed} 
              onChange={e => setSeed(e.target.value)}
              placeholder="Type to change genetics"
            />
          </div>
        ...
        <div className="control-group" style={{marginTop: '1rem'}}>
          <button 
            className={`btn ${isMicActive ? 'danger' : 'active'}`}
            onClick={toggleMic}
          >
